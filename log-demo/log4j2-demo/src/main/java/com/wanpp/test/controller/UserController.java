...
public class UserController {
    ...
    @ApiImplicitParams({
        @ApiImplicitParam(name = "paramsDTO", value = "实体参数信息", required = true, dataType = "UserDTO", paramType = "query"),
    ...
    public ResponseEntity<JSONObject> describeFuzzyList(@ModelAttribute UserDTO paramsDTO,
    ...
    @RequestParam(name = "pageSize", required = true) Integer pageSize) {
        ...
        try {
            ...
            //执行
            List<UserDTO> resultDTOList = userService.selectFuzzyByDto(paramsDTO);
    ...
    public ResponseEntity<JSONObject> describeList(@ModelAttribute UserDTO paramsDTO,
        @RequestParam(name = "currentPage", required = true) Integer currentPage,
        @RequestParam(name = "pageSize", required = true) Integer pageSize) {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "paramsDTO", value = "实体参数信息", required = true, dataType = "User", paramType = "body")})
    @RequestMapping(value = "/save", method = RequestMethod.POST)
    public ResponseEntity<JSONObject> save(@RequestBody UserDTO paramsDTO) {
        logger.info("准备执行 数据插入，传入的参数为：" + JSON.toJSONString(paramsDTO));
        JSONObject data = new JSONObject();
        try {
            //执行
            Integer result = userService.insertDto(paramsDTO);
    ...
    public ResponseEntity<JSONObject> update(@RequestBody UserDTO paramsDTO) {
        logger.info("准备执行 数据修改，传入的参数为：" + JSON.toJSONString(paramsDTO));
        JSONObject data = new JSONObject();
        try {
            //执行
            Integer result = userService.updateDto(paramsDTO);
            data.put("data",result);
            data.put("msg", "数据修改成功");
            data.put("code", ReturnCodeData.SUCCESS);
        } catch (Exception e) {
    ...
    public ResponseEntity<JSONObject> delete(@RequestBody UserDTO paramsDTO) {
        logger.info("准备执行 数据删除，传入的参数为：" + JSON.toJSONString(paramsDTO));
        JSONObject data = new JSONObject();
        try {
            //执行
            Integer result = userService.deleteDto(paramsDTO);
            data.put("data",result);
            data.put("msg", "数据删除成功");
            data.put("code", ReturnCodeData.SUCCESS);
        } catch (Exception e) {
